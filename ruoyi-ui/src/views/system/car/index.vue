<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="carList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="状态" align="center" key="status" v-hasPermi="['system:car:changeStatus']">
...
<script>
import {listCar, getCar, delCar, addCar, updateCar, changeCarStatus} from "@/api/system/car";
...
export default {
  ...
  methods: {
    ...
    handleStatusChange(row) {
      ...
      console.log(text)
      this.$modal.confirm('确认要' + text + +row.carNo+'号车【' + row.carNum + '】吗？').then(function() {
        return changeCarStatus(row.id, row.status);
      }).then(() => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除车辆管理编号为"' + ids + '"的数据项？').then(() => {
        this.loading = true;
        return delCar(ids);
      }).then(() => {
        this.loading = false;
